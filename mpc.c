...
void mpc_err_print(mpc_err_t *x) { mpc_err_print_to(x, stdout); }
...
static int mpc_parse_run(mpc_input_t *i, mpc_parser_t *p, mpc_result_t *r,
  ...
  case MPC_TYPE_MANY:
    ...
    *e = mpc_err_merge(i, *e, results[j].error);
    ...
    MPC_SUCCESS(mpc_parse_fold(i, p->data.repeat.f, j, (mpc_val_t **)results);
  ...
  case MPC_TYPE_MANY1:
    ...
    if (j == 0) {
      MPC_FAILURE(mpc_err_many1(i, results[j].error);
                  if (j >= MPC_PARSE_STACK_MIN) { mpc_free(i, results); });
    } else {
  ...
  case MPC_TYPE_OR:
    ...
    for (j = 0; j < p->data.or.n; j++) {
      if (mpc_parse_run(i, p->data.or.xs[j], &results[j], e, depth + 1)) {
        MPC_SUCCESS(results[j].output; if (p->data.or.n > MPC_PARSE_STACK_MIN) {
          mpc_free(i, results);
        });
      } else {
        *e = mpc_err_merge(i, *e, results[j].error);
  ...
  case MPC_TYPE_AND:
    ...
    for (j = 0; j < p->data.and.n; j++) {
      if (!mpc_parse_run(i, p->data.and.xs[j], &results[j], e, depth + 1)) {
        ...
        MPC_FAILURE(results[j].error; if (p->data.or.n > MPC_PARSE_STACK_MIN) {
          mpc_free(i, results);
        });
      }
    }
...
static void mpc_soft_delete(mpc_val_t *x) { mpc_undefine_unretained(x, 0); }
...
mpc_parser_t *mpc_real(void) {
  ...
  return mpc_expect(mpc_and(4, mpcf_strfold, p0, p1, p2, p3, free, free, free),
                    "real");
}
...
static mpc_val_t *mpcf_re_escape(mpc_val_t *x, void *data) {
  ...
  if (s[0] == '^') {
    ...
    if (mode & MPC_RE_MULTILINE) {
      return mpc_and(2, mpcf_snd, mpc_or(2, mpc_soi(), mpc_boundary_newline()),
...
mpc_parser_t *mpc_re(const char *re) { return mpc_re_mode(re, MPC_RE_DEFAULT); }
...
mpc_val_t *mpcf_escape_string_raw(mpc_val_t *x) {
  mpc_val_t *y =
      mpcf_escape_new(x, mpc_escape_input_raw_cstr, mpc_escape_output_raw_cstr);
  free(x);
  return y;
}
...
void mpc_ast_print(mpc_ast_t *a) { mpc_ast_print_depth(a, 0, stdout); }

void mpc_ast_print_to(mpc_ast_t *a, FILE *fp) { mpc_ast_print_depth(a, 0, fp); }
...
mpc_val_t *mpcf_fold_ast(int n, mpc_val_t **xs) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (as[i] && as[i]->children_num == 0) {
      ...
      mpc_ast_add_child(r,
                        mpc_ast_add_root_tag(as[i]->children[0], as[i]->tag));
...
mpc_parser_t *mpca_grammar_st(const char *grammar, mpca_grammar_st_t *st) {
  ...
  mpc_define(GrammarTotal, mpc_predictive(mpc_total(Grammar, mpc_soft_delete)));
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
  ...
  while (1) {
    ...
    /* Merge rhs `or` */
    if (p->type == MPC_TYPE_OR && p->data.or
            .xs[p->data.or.n - 1]
            ->type == MPC_TYPE_OR &&
                !p->data.or
            .xs[p->data.or.n - 1]
    ...
    /* Remove ast `pass` */
    if (p->type == MPC_TYPE_AND && p->data.and.n == 2 &&
        p->data.and.xs[0]->type == MPC_TYPE_PASS &&
        !p->data.and.xs[0]->retained && p->data.and.f == mpcf_fold_ast) {
      t = p->data.and.xs[1];
    ...
    if (p->type == MPC_TYPE_AND && p->data.and.n == 2 &&
        ...
        !p->data.and.xs[0]->retained && p->data.and.f == mpcf_strfold) {
      t = p->data.and.xs[1];
      mpc_delete(p->data.and.xs[0]);
      free(p->data.and.xs);
      free(p->data.and.dxs);
    ...
    /* Merge re lhs `and` */
    if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_strfold &&
        p->data.and.xs[0]->type == MPC_TYPE_AND &&
        !p->data.and.xs[0]->retained &&
...
void mpc_optimise(mpc_parser_t *p) { mpc_optimise_unretained(p, 1); }
